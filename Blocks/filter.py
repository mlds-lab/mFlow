...
def ColumnSelectFilter(*args, **kwargs):
    return node(function = __ColumnSelectFilter, args=args, kwargs=kwargs, name="Column Filter")    

def __ColumnSelectFilter(df, cols, key="dataframe"):
    df=df[key]
    df=df[cols]
    return({"dataframe":df})